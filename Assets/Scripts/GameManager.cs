...
public class GameManager : MonoBehaviour
{
    ...
    public BottleFlashController bottleFlashController;
...
public TMP_Text resultText;
    ...
    private void Awake()
    {
        ...
        if (bottleFlashController == null)
            bottleFlashController = FindObjectOfType<BottleFlashController>();

        EnsureStatusTextReference();
    ...
    {
        if (bottleFlashController != null)
            bottleFlashController.SetIdle();

        StartNewLevel();
    }

    private void EnsureStatusTextReference()
    {
        ...
        {
            if (allTexts[i] != null && allTexts[i].name.ToLower().Contains("status"))
            {
                statusText = allTexts[i];
                return;
            }
        }
        ...
        GameObject go = new GameObject("StatusText", typeof(RectTransform), typeof(TextMeshProUGUI));
    ...
    private void AddIngredient(int ingredientIndex1to6)
    {
        ...
        if (bottleFlashController != null)
            bottleFlashController.FlashOnly(ingredientIndex1to6);
